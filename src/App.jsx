...
const AnalyticsView = ({ goal, logs, onClose }) => {
  ...
  return (
    <div className="animate-in slide-in-from-bottom-8 duration-500 flex flex-col h-full max-w-4xl mx-auto pb-8">
      ...
      <button 
        onClick={onClose} 
        className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/5 text-white rounded-2xl transition-colors font-medium"
      >
        Close Analysis
      </button>
    </div>
    ...
    <GlassCard className="p-6 flex flex-col justify-between h-full group">
      <div className="flex justify-between items-start mb-6">
        ...
        <button 
          ...
          className="p-2 rounded-xl bg-white/5 hover:bg-indigo-500/20 hover:text-indigo-300 text-slate-500 transition-colors"
        >
          <BarChart2 size={18} />
        </button>
      </div>
      ...
      <div>
        ...
        <button 
          ...
          className="w-full py-3 rounded-xl font-semibold text-sm shadow-lg transition-all duration-300 bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:shadow-indigo-500/25 hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center space-x-2"
        >
          <Plus size={16} />
    ...
    <div className={`min-h-screen ${THEME.bg} text-slate-100 font-sans selection:bg-indigo-500/30 pb-20`}>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <button 
            onClick={() => setIsAddingGoal(true)} 
            className="group flex flex-col items-center justify-center min-h-[220px] border border-dashed border-white/10 rounded-3xl hover:bg-white/5 transition-all duration-300"
          >
            <div className="w-14 h-14 bg-slate-900 rounded-full flex items-center justify-center text-slate-500 group-hover:text-indigo-400 group-hover:scale-110 transition-all shadow-inner shadow-black/50">
              <Plus size={24} />
            </div>
      ...
      >
        <form onSubmit={handleLogSubmit}>
          ...
          <button 
            type="submit" 
            disabled={!logValue} 
            className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-2xl transition-colors shadow-lg shadow-indigo-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Update Progress
          </button>
        </form>
      ...
      >
        <form onSubmit={handleAddGoal} className="space-y-5">
          ...
          <button 
            type="submit" 
            className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-4 rounded-2xl mt-2 transition-colors shadow-lg shadow-indigo-900/20"
          >
            Create Goal
          </button>
        </form>
