      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-slate-500">
            {friends.length === 0 ? "Add friends to see their progress!" : "Your friends haven't logged any progress recently"}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {feedItems.map(item => (
            <div key={item.id} className="p-4 border-2 border-teal-200 rounded-xl hover:bg-teal-50 transition-colors">
              ...
              {item.notes && <p className="text-sm text-slate-500 italic">"{item.notes}"</p>}
            </div>
          ))}
        </div>
      )}
...
const FriendGoalsModal = ({ isOpen, onClose, friend, db }) => {
  ...
  const getProgressPercent = (goal) => {
    ...
    if (goal.type === 'personal-best' && goal.bestValue !== null) {
      ...
      const startingPoint = goal.current;
      const target = goal.target;
      
      if (higherIsBetter) {
        const totalRange = target - startingPoint;
        const currentProgress = goal.bestValue - startingPoint;
        return totalRange > 0 ? Math.min(100, (currentProgress / totalRange) * 100) : 0;
      } else {
        const totalRange = startingPoint - target;
        const currentProgress = startingPoint - goal.bestValue;
        return totalRange > 0 ? Math.min(100, (currentProgress / totalRange) * 100) : 0;
      }
    }
    return 0;
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {goals.map(goal => {
            ...
            const isAheadOfSchedule = percent >= timeProgress;
              ...
              <GlassCard key={goal.id} className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-600">
                      {goal.type === 'personal-best' && goal.bestValue !== null
                        ? `Best: ${goal.bestValue} ${goal.unit}`
                        : `${goal.current} / ${goal.target} ${goal.unit}`}
                    </p>
                    <p className="text-xs text-slate-500 capitalize mt-1">{goal.type.replace('-', ' ')}</p>
                  </div>
                  {goal.privacy === 'public' && <Globe size={16} className="text-slate-400" />}
                  {goal.privacy === 'friends' && <Users size={16} className="text-teal-600" />}
                </div>

                <div className="w-full bg-slate-200 h-3 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${
                      isAheadOfSchedule ? 'bg-gradient-to-r from-blue-400 to-cyan-500' : 'bg-gradient-to-r from-rose-400 to-red-500'
                    }`}
                    style={{ width: `${percent}%` }}
                  />
                </div>
                ...
                {goal.trackStreak && goal.streak > 0 && (
                  <div className="flex items-center space-x-2 mt-3">
                    ...
                    <span className="text-sm font-medium text-slate-700">{goal.streak} day streak!</span>
                  </div>
                )}
              </GlassCard>
            );
...
const WrappedView = ({ isOpen, onClose, goals, logs, friends, userProfile }) => {
  ...
  const wrappedData = useMemo(() => {
    ...
    const streakGoal = goals.reduce((best, goal) => {
      return (goal.streak || 0) > (best?.streak || 0) ? goal : best;
    }, null);

    const goalLogCounts = {};
    yearLogs.forEach(log => {
      goalLogCounts[log.goalId] = (goalLogCounts[log.goalId] || 0) + 1;
    });
    const mostActiveGoalId = Object.keys(goalLogCounts).reduce((a, b) => 
      goalLogCounts[a] > goalLogCounts[b] ? a : b, null
    );
    const mostActiveGoal = goals.find(g => g.id === mostActiveGoalId);
    ...
    yearLogs.forEach(log => {
      const day = new Date(log.timestamp.seconds * 1000).getDay();
      dayData[day]++;
    });
    const bestDay = dayData.indexOf(Math.max(...dayData));
    const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

    const pbGoals = goals.filter(g => g.type === 'personal-best' && g.bestValue !== null);
    const pbsAchieved = pbGoals.filter(g => {
      const goalYearLogs = yearLogs.filter(l => l.goalId === g.id);
      return goalYearLogs.length > 0;
    }).length;

    const sortedLogs = [...yearLogs].sort((a, b) => a.timestamp.seconds - b.timestamp.seconds);
    ...
    return {
      ...
      streakGoal,
      mostActiveGoal,
      mostActiveGoalLogs: goalLogCounts[mostActiveGoalId] || 0,
      bestMonth: monthNames[bestMonth],
      bestDay: dayNames[bestDay],
      pbsAchieved,
      firstLogDate: new Date(firstLog.timestamp.seconds * 1000),
      lastLogDate: new Date(lastLog.timestamp.seconds * 1000),
  ...
  const slides = [
    {
      ...
      content: (
        <div className="text-center text-white">
          <Sparkles size={80} className="mx-auto mb-8 animate-pulse" />
          <h1 className="text-6xl font-black mb-4 tracking-tight">Your 2025</h1>
          <h2 className="text-4xl font-bold mb-8">Wrapped</h2>
          <p className="text-xl opacity-90 max-w-md mx-auto">A year of goals, growth, and incredible progress</p>
        </div>
      )
    },
    {
      ...
      content: (
        <div className="text-center text-white">
          <Trophy size={64} className="mx-auto mb-8" />
          <h2 className="text-3xl font-bold mb-12">You Were Unstoppable</h2>
          <div className="space-y-8">
            <div>
              <div className="text-7xl font-black mb-2">{wrappedData.totalLogs}</div>
              <div className="text-xl opacity-90">Times you logged progress</div>
            </div>
            <div className="pt-8 border-t-2 border-white/20">
              <div className="text-5xl font-bold mb-2">{wrappedData.completedGoals}/{wrappedData.totalGoals}</div>
              <div className="text-xl opacity-90">Goals completed</div>
            </div>
          </div>
        </div>
      )
    ...
    {
      bg: 'bg-gradient-to-br from-orange-500 via-red-500 to-rose-500',
      content: (
        <div className="text-center text-white">
          {wrappedData.streakGoal && wrappedData.streakGoal.streak > 0 ? (
            <>
              <Flame size={64} className="mx-auto mb-8 animate-bounce" />
              <h2 className="text-3xl font-bold mb-12">Streak Master</h2>
              <div className="mb-8">
                <div className="text-8xl font-black mb-4">{wrappedData.streakGoal.streak}</div>
                <div className="text-2xl opacity-90">day streak on</div>
                <div className="text-3xl font-bold mt-4">{wrappedData.streakGoal.name}</div>
              </div>
              <p className="text-lg opacity-80 max-w-md mx-auto">That's the kind of consistency that changes lives! 🔥</p>
            </>
          ) : (
            <>
              <Target size={64} className="mx-auto mb-8" />
              <h2 className="text-3xl font-bold mb-12">Consistency is Key</h2>
              <p className="text-xl opacity-90 max-w-md mx-auto">Keep showing up every day - that's where the magic happens!</p>
            </>
          )}
        </div>
      )
    ...
    {
      bg: 'bg-gradient-to-br from-purple-500 via-indigo-500 to-blue-500',
      content: (
        <div className="text-center text-white">
          <Activity size={64} className="mx-auto mb-8" />
          <h2 className="text-3xl font-bold mb-12">Your Focus</h2>
          {wrappedData.mostActiveGoal && (
            <>
              <div className="text-4xl font-bold mb-6">{wrappedData.mostActiveGoal.name}</div>
              <div className="text-7xl font-black mb-4">{wrappedData.mostActiveGoalLogs}</div>
              <div className="text-xl opacity-90 mb-8">times logged</div>
              <p className="text-lg opacity-80 max-w-md mx-auto">This goal got the most love from you this year!</p>
            </>
          )}
        </div>
      )
    },
    {
      ...
      content: (
        <div className="text-center text-white">
          <Calendar size={64} className="mx-auto mb-8" />
          <h2 className="text-3xl font-bold mb-12">Your Best Times</h2>
          <div className="space-y-8">
            <div>
              <div className="text-5xl font-bold mb-2">{wrappedData.bestMonth}</div>
              <div className="text-xl opacity-90">was your strongest month</div>
            </div>
            <div className="pt-8 border-t-2 border-white/20">
              <div className="text-4xl font-bold mb-2">{wrappedData.bestDay}</div>
              <div className="text-xl opacity-90">was your most productive day</div>
            </div>
          </div>
        </div>
      )
    },
    {
      ...
      content: (
        <div className="text-center text-white">
          <Award size={64} className="mx-auto mb-8" />
          <h2 className="text-3xl font-bold mb-12">Breaking Records</h2>
          {wrappedData.pbsAchieved > 0 ? (
            <>
              <div className="text-8xl font-black mb-4">{wrappedData.pbsAchieved}</div>
              <div className="text-2xl opacity-90 mb-8">Personal Bests achieved</div>
              <p className="text-lg opacity-80 max-w-md mx-auto">You didn't just show up - you showed UP! 💪</p>
            </>
          ) : (
            <p className="text-xl opacity-90 max-w-md mx-auto">2026 is your year to break records!</p>
          )}
        </div>
      )
    },
    {
      ...
      content: (
        <div className="text-center text-white">
          <Sparkles size={80} className="mx-auto mb-8 animate-pulse" />
          <h2 className="text-5xl font-black mb-8">2026 Awaits</h2>
          <p className="text-2xl mb-12 max-w-2xl mx-auto leading-relaxed">
            You've proven what you're capable of.
            <br />
            <span className="font-bold">Now go make 2026 even better.</span>
          </p>
          <div className="inline-flex items-center space-x-2 bg-white/20 backdrop-blur px-6 py-3 rounded-full">
            <Trophy size={24} />
            <span className="font-bold text-lg">Keep crushing it, {userProfile?.displayName || 'Champion'}!</span>
          </div>
        </div>
      )
    }
  ...
  return (
    <div className="fixed inset-0 z-[60] overflow-hidden">
      ...
      <div className={`h-full w-full ${slides[currentSlide].bg} transition-all duration-500 flex items-center justify-center p-8`}>
        <div className="max-w-4xl w-full">{slides[currentSlide].content}</div>
      </div>
      ...
      <div className="absolute bottom-8 left-0 right-0 flex items-center justify-between px-8 max-w-4xl mx-auto">
        <button
          ...
          className={`p-4 rounded-full bg-white/20 backdrop-blur text-white transition-all ${
            currentSlide === 0 ? 'opacity-0 pointer-events-none' : 'hover:bg-white/30'
          }`}
        >
        ...
        {currentSlide === slides.length - 1 ? (
          <button
            onClick={onClose}
            className="px-6 py-3 rounded-full bg-white text-slate-900 font-bold hover:scale-105 transition-transform"
          >
            Close
          </button>
        ) : (
          <button
            onClick={nextSlide}
            className="p-4 rounded-full bg-white/20 backdrop-blur text-white hover:bg-white/30 transition-all"
          >
      ...
      <button
        onClick={onClose}
        className="absolute top-6 right-6 p-3 rounded-full bg-white/20 backdrop-blur text-white hover:bg-white/30 transition-all"
      >
        <X size={24} />
      </button>
    </div>
...
const RecentActivity = ({ logs, goals, onDeleteLog }) => {
        ...
        {recentLogs.map(log => (
          <div key={log.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={() => {
                  if (window.confirm(`Delete this log entry?\n\n+${log.value} ${log.goalUnit} for ${log.goalName}`)) {
                    onDeleteLog(log);
                  }
                }}
                className="p-2 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 transition-colors"
  ...
  const getProgressData = () => {
    ...
    switch (goal.type) {
      ...
      case 'target': {
        ...
        const isAhead = percent >= timeProgress;
      ...
      case 'personal-best': {
        ...
        if (hasBest) {
          ...
          isAhead = percent >= timeProgress;
        }
        
        const bestDisplay = hasBest 
          ? `Best: ${goal.bestValue} ${goal.unit} (from ${startingPoint} ${goal.unit})` 
          : `Starting: ${startingPoint} ${goal.unit} • Goal: ${goal.target} ${goal.unit}`;
      ...
      case 'countdown': {
        ...
        return { 
          percent, 
          isAhead, 
          display: isAhead ? `Complete! 🎉` : `${remaining.toLocaleString()} ${goal.unit} remaining`
        };
      }
    ...
    <GlassCard className="p-6 flex flex-col justify-between h-full group">
      <div className="flex justify-between items-start mb-4">
        <div className="flex-1">
          ...
          <p className={`text-sm ${THEME.textMuted} mb-1`}>{display}</p>
          <div className="flex items-center space-x-2">
        ...
        <div className="w-full bg-slate-200 h-3 rounded-full mb-4 overflow-hidden">
          <div 
            className={`h-full rounded-full transition-all duration-700 ease-out ${
              isAhead ? 'bg-gradient-to-r from-blue-400 to-cyan-500' : 'bg-gradient-to-r from-rose-400 to-red-500'
            }`}
            style={{ width: `${percent}%` }}
            role="progressbar"
...
export default function App() {
  ...
  const handleDeleteLog = useCallback(async (log) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, `artifacts/${appId}/users/${userId}/logs`, log.id));

      let updateData = {};
      
      switch (goal.type) {
        case 'personal-best':
          const remainingLogs = logs.filter(l => l.goalId === goal.id && l.id !== log.id);
          if (remainingLogs.length > 0) {
            ...
            updateData = { bestValue: null, bestDate: null };
          }
          break;
          
        case 'maintain':
          updateData = { current: increment(-log.value) };
          break;
  ...
  const handleUndoLastLog = useCallback(async () => {
    ...
    if (logToUndo) {
      await handleDeleteLog(logToUndo);
      setLastLogId(null);
    }
  }, [lastLogId, user, db, logs, handleDeleteLog, addToast]);
  ...
  const handleAddOrEditGoal = async (e) => {
    ...
    const goalData = {
      ...
      ...(editingGoal ? {} : { current: goalForm.type === 'personal-best' ? parseFloat(goalForm.current) || 0 : 0, streak: 0 })
    };
  ...
  return (
    <div className={`min-h-screen ${THEME.bg} ${THEME.textMain} font-sans pb-20`}>
        ...
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            ...
            <button 
              ...
              className="p-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white rounded-xl transition-all hover:shadow-lg"
              title="2025 Wrapped"
            >
              <Sparkles size={20} />
            </button>
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <RecentActivity logs={logs} goals={goals} onDeleteLog={handleDeleteLog} />
